...
<xsl:stylesheet
  ...
  <xsl:template match="/page">
    ...
    <html>
      ...
      <xsl:choose>
        <xsl:when test="$PAGE_MODE = 'frameset'">
          <xsl:call-template name="application-frameset"/>
        </xsl:when>
        <xsl:when test="$PAGE_MODE = 'frame'">
          <xsl:call-template name="application-frame"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="application-page"/>
        </xsl:otherwise>
      </xsl:choose>
    </html>
  </xsl:template>
  ...
  <xsl:template name="application-page-navigation">
    <xsl:if test="$PAPAYA_USER_AUTHORIZED and not($PAGE_MODE = 'installer')">
      <div id="pageNavigation">
        <a href="#pageMenuBar">Menu</a>
        <xsl:text> * </xsl:text>
        <xsl:if test="leftcol">
          <a href="#pageNavigationColumn">Navigation</a>
          <xsl:text> * </xsl:text>
        </xsl:if>
        ...
        <xsl:otherwise>
          ...
          <table class="columnGrid">
            <tr>
              ...
              <xsl:choose>
                <xsl:when
                  test="$rightColumn and (($COLUMNWIDTH_RIGHT = '100%') or ($COLUMNWIDTH_RIGHT = $COLUMNWIDTH_CENTER))">
      ...
      <xsl:if test="$PAGE_USER != ''">
        ...
        <span class="user">
          ...
          <xsl:if test="$PAPAYA_MESSAGES_INBOX_NEW &gt; 0">
            (
            <a href="{$PAPAYA_MESSAGES_INBOX_LINK}">
              <xsl:value-of select="$PAPAYA_MESSAGES_INBOX_NEW"/>
            </a>
            )
          </xsl:if>
        </span>
      ...
      <script type="text/javascript">
        <xsl:comment>
          <xsl:for-each select="title-menu/links[@align = 'right']/link">
            ...
            PapayaRichtextSwitch.add("<xsl:value-of select="@title"/>", "<xsl:value-of select="@href"/>", <xsl:value-of
            select="$selected"/>);
          </xsl:for-each>
          PapayaRichtextSwitch.output(document.getElementById("titleMenu"), "<xsl:value-of
          select="title-menu/links[@align = 'right']/@title"/>");
          //
        </xsl:comment>
      </script>
    </xsl:if>
  ...
  <xsl:template name="application-page-links-icon">
    ...
    <xsl:variable name="imageSource">
      <xsl:choose>
        <xsl:when test="contains($image, '/icon.') or contains($image, '/i18n-icon.')">
          <xsl:value-of select="string($image)"/>
        </xsl:when>
        <xsl:when test="starts-with($image, 'icon.') or starts-with($image, 'i18n-icon.')">
          <xsl:value-of select="string($image)"/>
        </xsl:when>
        <xsl:when test="string($image) != ''">
          <xsl:text>pics/language/</xsl:text>
          <xsl:choose>
            <xsl:when test="contains(image, '.gif')">
              <xsl:value-of select="substring-before($image, '.gif')"/>
  ...
  <xsl:template name="application-page-links">
    <xsl:if test="title-menu/links[not(@align)]/@title">
      -
      <ul class="links">
        <li class="caption"><xsl:value-of select="title-menu/links[not(@align)]/@title"/>:
        </li>
  ...
  <xsl:template name="richtext-embed">
    <xsl:if test="$PAPAYA_USE_RICHTEXT and $EMBED_TINYMCE">
      ...
      <xsl:variable name="language-short">
        <xsl:choose>
          <xsl:when test="substring-before($PAPAYA_UI_LANGUAGE, '-')">
            <xsl:value-of select="substring-before($PAPAYA_UI_LANGUAGE, '-')"/>
          </xsl:when>
